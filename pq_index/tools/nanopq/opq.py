...
class OPQ(object):
    ...
    def fit(self, vecs, pq_iter=20, rotation_iter=10, seed=123):
        ...
        for i in range(rotation_iter):
            ...
            U, s, V = np.linalg.svd(np.matmul(vecs.T, X_))
    ...
    def rotate(self, vecs):
        ...
        if vecs.ndim == 2:
            # return vecs @ self.R
            return np.matmul(vecs, self.R)
        elif vecs.ndim == 1:
            # return (vecs.reshape(1, -1) @ self.R).reshape(-1)
            return np.matmul((vecs.reshape(1, -1), self.R)).reshape(-1)
    ...
    def decode(self, codes):
        ...
        return np.matmul(self.pq.decode(codes), self.R.T)
