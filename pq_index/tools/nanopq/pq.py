...
class PQ(object):
    ...
    def __init__(self, M, Ks=256, verbose=True):
        ...
        if verbose:
            logging.info("M: {}, Ks: {}, code_dtype: {}".format(M, Ks, self.code_dtype))
    ...
    def encode(self, vecs):
        ...
        for m in range(self.M):
            if self.verbose:
                logging.info("Encoding the subspace: {} / {}".format(m, self.M))
            vecs_sub = vecs[:, m * self.Ds : (m+1) * self.Ds]
