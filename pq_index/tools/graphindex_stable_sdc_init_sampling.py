...
def read_one_file(fp_path, fp, cnt, emb_list, terms):
    with open(os.path.join(fp_path, fp), 'r') as f:
        for i in f:
            lines = i.strip().split('\t')
            if len(lines) != 2:
                continue
...
class PQINDEX(object):
    ...
    @staticmethod
    def read_term_emb_by_file(fp_path):
        terms = []
        emb_list = []
        cnt = 0
    ...
    @staticmethod
    def read_term_emb_by_file_parallel(fp_path, phrase='train', sampling_rate=0.2, threads_num=512):
        terms = []
        emb_list = []
        cnt = 0
        fp_list = os.listdir(fp_path)
        random.Random(4).shuffle(fp_list)
        if phrase == 'train':
            fp_list = fp_list[:int(len(fp_list) * sampling_rate)] 
        else:
            fp_list = fp_list[int(len(fp_list) * sampling_rate):]
        print(len(fp_list))
    ...
    def prepare_opqindex_by_file(self, input_fp, output_fp, codewords_fp, M=8, Ks=20):
        """
        ...
        start = time.time()
        self.id2pid, emb_list = self.read_term_emb_by_file_parallel(input_fp)
        ...
        logger.info('begin read file[' + input_fp + ']')
        start = time.time()
        self.id2pid, emb_list = self.read_term_emb_by_file_parallel(input_fp, phrase='infer')
        logger.info('read file time {}'.format(time.time()-start))
        self.emb_list = np.array(emb_list).astype(np.float32)
        logger.info('emb_list.shape:' + str(self.emb_list.shape))
        ...
        with open(codewords_fp, 'w') as f:
            for i in range(M):
                for j in range(Ks):
                    ...
                    f.write('\t'.join(tmp))
                    f.write('\n')
        with open(codewords_fp + '_RMatrix', 'w') as f:
...
if __name__ == '__main__':
    ...
    if first_init:
        ...
        fp_output = './init_index/pid2path.txt' + '_' + task_name
        fp_output_unique = './init_index/em_pid2path.txt_r0' + '_' + task_name
        fp_codewords = './init_index/codesbook.txt' + '_' + task_name
        fp_Rmatrix = './init_index/codesbook.txt' + '_' + task_name + '_RMatrix'
        pqdm.prepare_opqindex_by_file(pid_emb_path, fp_output, fp_codewords, M, Ks)
        pqdm.jtm_from_file(pid_emb_path, fp_output_unique, fp_output, fp_codewords, fp_Rmatrix, M, Ks)
